...
決定: A + dedicated opt-in を選択した。operator-facing な名前は `v6 inspection mount` とし、read-only は
実行時の安全性質として扱う。現時点で inspection mount の足場があり、write path より失敗時の影響範囲を狭くできる。
...
- env-gated smoke ではなく、通常の明示 inspection admission の regression を追加する。
...
- v6 inspection mount は inspection / smoke / cutover rehearsal 用に限定する。
