...
次は `SDW-V6RT-T10 v6 write opt-in parser and fail-closed gate` に進む。T4-T9 で journal、
live metadata routing、delayed/background mutation policy、post-write fsck policy、lock/stress gate、
operator cutover boundary を固定したので、次の最小実装は user-visible opt-in を parser に追加し、未対応条件を
fail closed する regression である。

T10 でも通常の v6 write mount は暗黙有効化しない。成功 path を入れる場合でも `rw,v6_write_mount` の
明示 opt-in、unsupported option rejection、T4-T9 closeout gate を必須にする。
