...
次は `SDW-V6RT-T9 v6 explicit write opt-in cutover boundary` に進む。T4-T8 で journal、
live metadata routing、delayed/background mutation policy、post-write fsck policy、lock/stress gate の
境界を固定したので、残る判断材料は operator cutover、rollback、unsupported option、fail-closed admission
boundary である。

T9 でも通常の v6 write mount は暗黙有効化しない。user-visible opt-in 名と拒否条件を固定し、operator が
write 前後に実行する fsck と rollback 手順を文書化してから、実際の mount admission wiring に進む。
