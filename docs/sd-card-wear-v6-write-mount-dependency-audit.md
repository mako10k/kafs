...
| Pending log | v6 descriptor には pending-log shard があるが、runtime は superblock offset 参照。 | v6 write mount では pending log を明示無効化するか、descriptor-backed pending-log view を実装して replay/drain/worker を検証する。 |
| Tail metadata | v5 tail metadata path は存在するが v6 routing はない。 | v6 write mount では tail packing / tail normalization / tombstone tail reclaim を無効化するか、v6 tail-metadata shard を生成・検証・routing する。 |
| Background workers | read-only context では起動しない。write context では pending worker、tombstone GC、background dedup が起動し得る。 | v6 write admission では各 worker を disabled default にするか、descriptor-backed metadata routing と lock/stress tests を先に通す。 |
