...
次は `SDW-V6RT-T8 v6 write mount lock/stress gate` に進む。T4-T7 で journal、live metadata routing、
delayed/background mutation policy、post-write fsck policy の境界を固定したので、残る write admission 前
ブロッカーは lock rank order と contention/stress regression である。

T8 でも write mount はまだ暗黙有効化しない。explicit opt-in の候補 path を lock policy に照らして監査し、
少なくとも concurrent write regression を 1 つ追加してから write admission 判断に進む。
