...
- `fsck.kafs` は v6 image で detect-only validation のみを supported path とし、repair/write option が
  指定された場合、`format v6 repair/write modes are not supported yet` として拒否する。
...
| --- | --- | --- |
| Admission / open mode | inspection mount は `O_RDONLY`、`PROT_READ`、read lock、FUSE `ro`。handoff は write mapping を診断だけ行う。T10 で `--v6-write-mount` / `-o v6_write_mount` / `-o v6-write-mount` は parser で認識し、未実装条件を fail closed する gate を追加した。 | controlled write admission 成功 path を入れる前に、user-visible FUSE write surface の許可範囲を function 単位で固定する。通常 v6 mount を暗黙 write admission にしない。 |
...
| fsck / repair | v6 detect-only validation は可能。write/repair modes は拒否。T7 で required command は `fsck.kafs --balanced-check <image>` と決定した。 | write mount 後の unclean state を detect-only で fail closed できる状態を維持し、repair write は descriptor-backed journal reset、replica repair、metadata shard repair の別段階で解禁する。 |
| Locking | `.github/lock-policy.md` は rank order と `KAFS_CALL` after lock 禁止を要求する。T8 で explicit write opt-in 候補の低レイヤ path は rank order 監査と contention regression を完了した。 | user-visible FUSE write surface 全体は引き続き PR/release gate で lock policy に照らして確認する。 |
| Operator cutover | v6 destination は offline staging と inspection mount まで。T9 で controlled write opt-in 名、fail-closed 条件、required fsck、rollback、release note draft を固定した。 | 実装時は T9 の boundary を parser/admission regression に落とし込み、通常 v6 mount を暗黙 write admission にしない。 |
...
5. Lock-order and stress validation (T8 complete for the explicit opt-in candidate lower-layer path)
6. Operator cutover and rollback documentation (T9 complete)

## 推奨する次タスク

次は `SDW-V6RT-T11 v6 FUSE write surface admission audit` に進む。T10 で user-visible opt-in は parser で
認識されるが、成功 path はまだ `controlled write mount is not enabled yet` として fail closed する。
成功 path を入れる前に create/write/truncate/fallocate/unlink/rename/link/symlink/copy/reflink/fsync/release
の各 path を function 単位で監査し、初期 controlled opt-in で許可する operation と拒否する operation を
固定する。

T11 でも通常の v6 write mount は暗黙有効化しない。成功 path を入れる場合でも `rw,v6_write_mount` の
明示 opt-in、unsupported option rejection、T4-T10 closeout gate を必須にする。
