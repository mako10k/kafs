...
- first-cut の現時点推奨は dedicated tail metadata region + inode growth である
...
### Current First-Cut Recommendation

follow-up design issue の現時点結論として、初回実装の第一候補は次の組み合わせである。

1. tail metadata は allocator reserved area に載せず、dedicated tail metadata region を持つ
2. tail descriptor 追加に伴う inode 変更は、inline payload を削らず inode size growth で吸収する

理由:

- tail metadata の ownership / fsck scan boundary を allocator から分離できる
- offline migration で tail region を空初期化しやすい
- `KAFS_DIRECT_SIZE` の意味を保ったまま format bump を設計できる
- old inline file が migration 後に境界超過するケースを避けやすい

この文書の以降の案は、この first-cut recommendation を前提に読むのが自然である。
...
初回導入で inode 内固定長 descriptor を採るなら、descriptor 自体は 14 bytes から始めるのが妥当である。
...
- 114-byte v4 inode へ追加しても 128-byte target に収まる
...
ここでの first-cut recommendation は、inline payload を削るのではなく inode size 自体を増やす案である。

### Byte-Budget Tradeoff

inode に tail descriptor を追加すると、設計上は次のどちらかになる。

- inode サイズを増やして inline payload を維持する
- inode 総サイズを固定し、inline payload を descriptor 分だけ減らす
...
### Current Geometry Cost Sketch

現行 v4 inode は packed layout で 114 bytes である。

内訳:

- fixed metadata fields: 54 bytes
- `i_blkreftbl[15]`: 60 bytes

ここに 14-byte tail descriptor を inode growth で追加すると、新 inode は 128 bytes になる。

- current inode bytes: 114
- proposed inode bytes: 128
- delta per inode: 14
- relative growth: 約 12.3%
- `KAFS_DIRECT_SIZE`: 60 bytes のまま維持できる

典型 inode count での inode table 増分は概ね次のとおりである。

| inode count | current table bytes | grown table bytes | delta |
|-------------|---------------------|-------------------|-------|
...
運用上の第一候補は in-place 変換ではなく、`kafsresize --migrate-create` 系の offline rebuild workflow とする。

狙い:

- 旧 image を最後まで rollback 元として残せる
- inode layout 変更と tail arena metadata 初期化を分離しやすい
- 途中失敗時に source image を汚さない

推奨シーケンス:

1. source image を unmount する
2. source image のバックアップを取る
3. new format を持つ destination image を作る
4. inode / directory / data を destination へ copy-on-write 的に再配置する
5. regular file は初回 migration では full-block layout のまま移す
6. superblock の active format を destination 側で確定する
7. destination image に対して fsck を実行する
8. 検証後に mount target を destination へ切り替える

### Why Not In-Place Migration First

初回導入で in-place migration を第一案にしない理由:

- inode shape の変更と data relocation が 1 つの失敗面になる
- tail container 初期化途中で crash したとき rollback が難しい
- old/new descriptor 解釈が同じ image 内で混在しやすい
- fsck が upgrade 中間状態まで背負うと責務が重くなる

### First-Cut Migration Policy

初回 migration policy は保守的に置く。

- migrate 時点で既存 regular file を tail 化しない
- migrate 完了直後の image では layout 互換だけを合わせる
- tail arena 利用は migrate 後の新規 file / overwrite / append から始める
- bulk repack は別の offline maintenance step として分離する

### Inline Threshold Migration Expectations

inode growth 案を採る場合、migration 直後の inline threshold は current と同じ 60 bytes のまま維持できる。

この前提だと migration 境界は単純である。

- old image で 0..60 bytes の inline regular file は、new image でも inline のまま保持できる
- old image で 61 bytes 以上の regular file は、migration 中に新しい tail descriptor を強制しなくてよい
- 「old inline だったが new threshold では入らない」という境界ケースは発生しない

したがって初回 migration は layout 互換の変換に集中でき、tail packing の policy 判断を migrate path へ持ち込まずに済む。

逆に inode 非拡張案では、threshold が 60 bytes から 44 bytes へ縮むため、45..60 bytes の既存 inline file が migration 時に新たな昇格対象になる。

この 45..60 bytes 帯は small-file fast path の恩恵を受けている範囲でもあり、初回 format bump の副作用として増やす理由は弱い。

### Post-Migration Validation

destination image に対して最低限必要な確認:

- inode count / directory reachability が source と一致する
- full-block file の logical size と block refs が source と一致する
- tail arena metadata が空初期化されているか、もしくは policy どおり未使用である
