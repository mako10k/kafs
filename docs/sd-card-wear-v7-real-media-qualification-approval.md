...
direct/single/double-indirect growth, shrink above the inline boundary or to zero,
double-to-single-to-direct truncate, non-zero direct-to-inline rejection, `O_TRUNC`,
...
T49 added regular-file promotion, T53 added the single-indirect lifecycle, and
T54 added the double-indirect lifecycle while this matrix remained `DRAFT`.
Any previously calculated draft digest is therefore obsolete; later operator
approval must bind the byte-for-byte digest of the completed matrix that
includes all three workloads.
