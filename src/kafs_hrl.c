...
static inline uint32_t *hrl_index_tbl(kafs_context_t *ctx) {
  return (uint32_t*)ctx->c_hrl_index;
}
...
static inline uint32_t hrl_bucket_count(kafs_context_t *ctx) {
  return ctx->c_hrl_bucket_cnt;
}
...
static int hrl_release_blo(kafs_context_t *ctx, kafs_blkcnt_t *pblo) {
  if (!pblo || *pblo == KAFS_BLO_NONE) return 0;
  kafs_blksize_t bs = hrl_blksize(ctx);
  char z[bs]; memset(z, 0, bs);
...
static int hrl_entry_cmp_blo(kafs_context_t *ctx, kafs_hrl_entry_t *e, kafs_blkcnt_t blo, const void *buf) {
  if (e->blo != blo) return 0;
  // confirm equality by reading and comparing when buffer provided
  if (!buf) return 1;
  kafs_blksize_t bs = hrl_blksize(ctx);
  char tmp[bs];
  if (hrl_read_blo(ctx, blo, tmp) != 0) return 0;
  return memcmp(tmp, buf, bs) == 0;
}

static int hrl_entry_cmp_content(kafs_context_t *ctx, kafs_hrl_entry_t *e, const void *buf, uint64_t fast) {
  if (e->fast != fast) return 0;
  kafs_blksize_t bs = hrl_blksize(ctx);
  char tmp[bs];
  if (hrl_read_blo(ctx, e->blo, tmp) != 0) return 0;
  return memcmp(tmp, buf, bs) == 0;
}

static int hrl_find_by_hash(kafs_context_t *ctx, uint64_t fast, const void *buf, uint32_t *out_index) {
...
static int hrl_find_free_slot(kafs_context_t *ctx, uint32_t *out_index) {
  ...
  for (uint32_t i = 0; i < cap; ++i) {
    if (ents[i].refcnt == 0) { *out_index = i; return 0; }
  }
  return -ENOSPC;
}
...
static int hrl_chain_remove(kafs_context_t *ctx, uint32_t idx, uint64_t fast) {
  ...
  while (head != 0) {
    ...
    if (i == idx) {
      ...
      if (prev == 0) index[b] = next; else ents[prev - 1u].next_plus1 = next;
      return 0;
    }
    prev = head;
...
int kafs_hrl_open(kafs_context_t *ctx) {
  if (!ctx || !ctx->c_superblock) return -EINVAL;
  uintptr_t base = (uintptr_t)ctx->c_superblock;
...
int kafs_hrl_put(kafs_context_t *ctx, const void *block_data, kafs_hrid_t *out_hrid, int *out_is_new, kafs_blkcnt_t *out_blo) {
  if (!ctx || !block_data || !out_hrid || !out_is_new || !out_blo) return -EINVAL;
  if (ctx->c_hrl_bucket_cnt == 0 || hrl_capacity(ctx) == 0) return -ENOSYS;
  ...
  // allocate entry
  if (hrl_find_free_slot(ctx, &idx) != 0) return -ENOSPC;
...
int kafs_hrl_inc_ref(kafs_context_t *ctx, kafs_hrid_t hrid) {
  ...
  if (e->refcnt == 0xFFFFFFFFu) return -EOVERFLOW;
  e->refcnt += 1u;
  return 0;
}
...
int kafs_hrl_lookup(kafs_context_t *ctx, const kafs_hr_digest_t *dg, kafs_hrid_t *out_hrid) {
  (void)dg; (void)ctx; (void)out_hrid; return -ENOSYS; // not used in this implementation
}
...
int kafs_hrl_write_block(kafs_context_t *ctx, const void *buf, kafs_hrid_t *out_hrid, int *out_is_new) {
  kafs_blkcnt_t blo;
...
static int kafs_hrl_find_by_blo(kafs_context_t *ctx, kafs_blkcnt_t blo, kafs_hrid_t *out_hrid) {
  ...
  if (hrl_find_by_hash(ctx, fast, buf, &idx) == 0) {
    kafs_hrl_entry_t *e = hrl_entries_tbl(ctx) + idx;
    if (hrl_entry_cmp_blo(ctx, e, blo, buf)) { *out_hrid = idx; return 0; }
  }
  return -ENOENT;
}
...
int kafs_hrl_dec_ref_by_blo(kafs_context_t *ctx, kafs_blkcnt_t blo) {
  ...
  if (rc == 0) return kafs_hrl_dec_ref(ctx, hrid);
