...
static const char *pick_kafs_exe(void) {
  ...
  for (int i = 0; cands[i]; ++i) {
    ...
    if (strchr(c, '/')) {
      if (access(c, X_OK) == 0) { strncpy(chosen, c, sizeof(chosen) - 1); chosen[sizeof(chosen)-1] = '\0'; return chosen; }
    } else {
      return c;
    }
  }
...
static int run_cmd(char *const argv[]) {
  ...
  int status = 0; if (waitpid(pid, &status, 0) < 0) return -errno;
...
static pid_t mount_kafs(const char *img, const char *mnt) {
  ...
  if (lfd >= 0) { dup2(lfd, STDERR_FILENO); dup2(lfd, STDOUT_FILENO); close(lfd); }
    ...
    execvp(kafs_exe, args); perror("execvp kafs"); _exit(127);
  }
...
int main(void) {
  ...
  if (kafs_test_mkimg_with_hrl(img, 64u*1024u*1024u, 12 /*4KiB*/, 4096, &ctx, &mapsize) != 0) {
    tlogf("mkimg failed"); return 1;
  }
  munmap(ctx.c_superblock, mapsize); close(ctx.c_fd);

  pid_t srv = mount_kafs(img, mnt);
  if (srv <= 0) { tlogf("mount failed (fuse perms?)"); return 77; }

  // Ensure root of mount is writable by current user
  uid_t uid = getuid(); gid_t gid = getgid();
  if (chown(mnt, uid, gid) != 0) { tlogf("chown mount root failed: %s", strerror(errno)); }
  if (chmod(mnt, 0777) != 0) { tlogf("chmod mount root failed: %s", strerror(errno)); }

  // 1) mkdir -p .git/hooks
  char path[PATH_MAX];
  snprintf(path, sizeof(path), "%s/.git", mnt);
  if (mkdir(path, 0777) != 0 && errno != EEXIST) { tlogf("mkdir .git failed: %s", strerror(errno)); unmount_kafs(mnt, srv); return 1; }
  snprintf(path, sizeof(path), "%s/.git/hooks", mnt);
  if (mkdir(path, 0777) != 0 && errno != EEXIST) { tlogf("mkdir .git/hooks failed: %s", strerror(errno)); unmount_kafs(mnt, srv); return 1; }
  ...
  if (lstat(p1, &st1) == 0) tlogf("TYPE .git: %s", S_ISDIR(st1.st_mode) ? "dir" : (S_ISREG(st1.st_mode)?"reg":"other"));
  if (lstat(p2, &st2) == 0) tlogf("TYPE .git/hooks: %s", S_ISDIR(st2.st_mode) ? "dir" : (S_ISREG(st2.st_mode)?"reg":"other"));
  ...
  if (fd < 0) {
    ...
    if (lf) {
      fseek(lf, 0, SEEK_END); long sz = ftell(lf); long tail = 4000; if (sz < tail) tail = sz; fseek(lf, -tail, SEEK_END);
      char buf[512]; size_t n; tlogf("--- server log tail ---");
      while ((n = fread(buf, 1, sizeof(buf), lf)) > 0) fwrite(buf, 1, n, stderr);
      fclose(lf);
    }
    unmount_kafs(mnt, srv); return 1;
  }
  const char *data = "#!/bin/sh\nexit 0\n"; (void)!write(fd, data, (unsigned)strlen(data)); close(fd);

  // 3) mkdir/rmdir empty
  snprintf(path, sizeof(path), "%s/empty", mnt);
  if (mkdir(path, 0700) != 0) { tlogf("mkdir empty failed: %s", strerror(errno)); unmount_kafs(mnt, srv); return 1; }
  if (rmdir(path) != 0) { tlogf("rmdir empty failed: %s", strerror(errno)); unmount_kafs(mnt, srv); return 1; }
