...
static void stop_kafs(const char *mnt, pid_t pid) {
  ...
  // 2) 通常のアンマウントを数回試行
  for (int i = 0; i < 5; ++i) {
    if (!is_still_mounted(mnt)) break;
    char *um1[] = {"fusermount3", "-u", (char *)mnt, NULL};
    if (run_cmd(um1) == 0 && !is_still_mounted(mnt)) break;
    // 少し待つ
...
int main(void) {
  ...
  if (kafs_test_mkimg_with_hrl(img, 16u*1024u*1024u, 12, 2048, &ctx, &mapsize) != 0) { return 77; }
  munmap(ctx.c_superblock, mapsize); close(ctx.c_fd);

  pid_t srv = spawn_kafs(img, mnt);
  if (srv <= 0) { return 77; }

  char p[PATH_MAX]; snprintf(p, sizeof(p), "%s/file.txt", mnt);
  int fdw = open(p, O_CREAT|O_TRUNC|O_WRONLY, 0644); if (fdw<0){ stop_kafs(mnt, srv); return 1; }
  write(fdw, "abcdef", 6); close(fdw);

  int fdr = open(p, O_RDONLY); if (fdr<0){ stop_kafs(mnt, srv); return 1; }
  // path unlink
  if (unlink(p)!=0){ stop_kafs(mnt, srv); return 1; }
  // 読めること
  char buf[16]={0}; ssize_t r = read(fdr, buf, sizeof(buf)); if (r <= 0){ tlogf("read after unlink failed"); close(fdr); stop_kafs(mnt, srv); return 1; }
  close(fdr);
  // パスは消えている
  if (access(p, F_OK) == 0){ tlogf("path still exists"); stop_kafs(mnt, srv); return 1; }
