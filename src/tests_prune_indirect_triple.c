...
int main(void) {
  ...
  char *buf = malloc(bs); memset(buf, 0xEF, bs);
  ssize_t w = pwrite(fd, buf, bs, p_off); free(buf);
  if (w != (ssize_t)bs) { tlogf("pwrite TI data failed: %s", strerror(errno)); close(fd); stop_kafs(mnt, srv); return 1; }

  char *z = calloc(1, bs); w = pwrite(fd, z, bs, p_off); free(z);
  if (w != (ssize_t)bs) { tlogf("pwrite TI zero failed: %s", strerror(errno)); close(fd); stop_kafs(mnt, srv); return 1; }
  fsync(fd); close(fd);
  ...
  // After prune, i_blkreftbl[14] (triple-indirect top table) may be NONE if its only child path was cleared
  ...
  munmap(base, mapsize); close(ifd); tlogf("prune_indirect_triple OK"); return 0;
}
