...
kafs_sb_load_blkcnt (const kafs_ssuperblock_t * sb)
{
  assert (sb != NULL);
...
kafs_is_zero_inocnt_free (const kafs_ssuperblock_t * sb)
{
  assert (sb != NULL);
...
kafs_sb_load_inocnt (const kafs_ssuperblock_t * sb)
{
  assert (sb != NULL);
...
__attribute_maybe_unused__ static int
kafs_load_inocnt_free (const kafs_ssuperblock_t * sb)
{
  assert (sb != NULL);
...
static kafs_blkcnt_t
kafs_sb_load_r_blkcnt (const struct kafs_ssuperblock *sb)
{
  assert (sb != NULL);
  return kafs_blkcnt_stoh (sb->s_blkcnt);
}
...
static kafs_blkcnt_t
kafs_sb_load_blkcnt_free (const struct kafs_ssuperblock *sb)
{
  assert (sb != NULL);
...
static void
kafs_sb_save_blkcnt_free (struct kafs_ssuperblock *sb, kafs_blkcnt_t blkcnt_free)
{
  assert (sb != NULL);
...
static kafs_blkcnt_t
kafs_sb_incr_blkcnt_free (struct kafs_ssuperblock *sb)
{
  assert (sb != NULL);
...
static void
kafs_sb_save_wtime (struct kafs_ssuperblock *sb, kafs_time_t wtime)
{
  assert (sb != NULL);
...
kafs_sb_load_log_blksize (const struct kafs_ssuperblock *sb)
{
  assert (sb != NULL);
...
kafs_sb_load_blksize (const struct kafs_ssuperblock *sb)
{
  assert (sb != NULL);
  return 1 << kafs_sb_load_log_blksize (sb);
}
