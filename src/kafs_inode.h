...
static inline void kafs_inode_zero_for_format(void *inoent, uint32_t format_version)
{
  ...
  if (format_version == KAFS_FORMAT_VERSION_V5 || format_version == KAFS_FORMAT_VERSION_V7)
  {
...
KAFS_MAYBE_UNUSED static kafs_uid_t kafs_ino_uid_get(const kafs_sinode_t *inoent)
{
  return kafs_uid_stoh(inoent->i_uid);
}

KAFS_MAYBE_UNUSED static void kafs_ino_uid_set(kafs_sinode_t *inode, kafs_uid_t uid)
{
  inode->i_uid = kafs_uid_htos(uid);
}

KAFS_MAYBE_UNUSED static kafs_gid_t kafs_ino_gid_get(const kafs_sinode_t *inoent)
{
  return kafs_gid_stoh(inoent->i_gid);
}

KAFS_MAYBE_UNUSED static void kafs_ino_gid_set(kafs_sinode_t *inode, kafs_gid_t gid)
{
  inode->i_gid = kafs_gid_htos(gid);
}

KAFS_MAYBE_UNUSED static kafs_dev_t kafs_ino_dev_get(const kafs_sinode_t *inoent)
{
  return kafs_dev_stoh(inoent->i_rdev);
}

KAFS_MAYBE_UNUSED static void kafs_ino_dev_set(kafs_sinode_t *inode, kafs_dev_t dev)
{
  inode->i_rdev = kafs_dev_htos(dev);
}

KAFS_MAYBE_UNUSED static kafs_blkcnt_t kafs_ino_blocks_get(const kafs_sinode_t *inoent)
{
  return kafs_blkcnt_stoh(inoent->i_blocks);
}

KAFS_MAYBE_UNUSED static void kafs_ino_blocks_set(kafs_sinode_t *inode, kafs_blkcnt_t blkcnt)
{
  inode->i_blocks = kafs_blkcnt_htos(blkcnt);
}

KAFS_MAYBE_UNUSED static kafs_off_t kafs_ino_size_get(const kafs_sinode_t *inoent)
{
  assert(inoent != NULL);
  return kafs_off_stoh(inoent->i_size);
}

KAFS_MAYBE_UNUSED static void kafs_ino_size_set(kafs_sinode_t *inoent, kafs_off_t size)
{
  assert(inoent != NULL);
  inoent->i_size = kafs_off_htos(size);
}

KAFS_MAYBE_UNUSED static kafs_time_t kafs_ino_atime_get(const kafs_sinode_t *inoent)
{
  assert(inoent != NULL);
  return kafs_time_stoh(inoent->i_atime);
}

KAFS_MAYBE_UNUSED static void kafs_ino_atime_set(kafs_sinode_t *inoent, kafs_time_t atime)
{
  assert(inoent != NULL);
...
KAFS_MAYBE_UNUSED static kafs_time_t kafs_ino_dtime_get(const kafs_sinode_t *inoent)
{
  assert(inoent != NULL);
