...
int main(void) {
  ...
  int fd = open(p, O_CREAT|O_WRONLY, 0644); if (fd<0){ tlogf("create file failed:%s", strerror(errno)); stop_kafs(mnt, srv); return 1; } close(fd);
  snprintf(p, sizeof(p), "%s/file/child", mnt);
  if (mkdir(p, 0700) == 0 || errno != ENOTDIR) { tlogf("expected ENOTDIR, got %d", errno); stop_kafs(mnt, srv); return 1; }

  // 2) O_TRUNC on existing file
  snprintf(p, sizeof(p), "%s/trunc", mnt);
  fd = open(p, O_CREAT|O_WRONLY, 0644); if (fd<0){ tlogf("create trunc failed:%s", strerror(errno)); stop_kafs(mnt, srv); return 1; }
  const char *data = "abc"; write(fd, data, 3); close(fd);
  fd = open(p, O_WRONLY|O_TRUNC, 0644); if (fd<0){ tlogf("open O_TRUNC failed:%s", strerror(errno)); stop_kafs(mnt, srv); return 1; } close(fd);
  struct stat st={0}; if (stat(p,&st)!=0 || st.st_size!=0){ tlogf("truncate not applied size=%ld", (long)st.st_size); stop_kafs(mnt, srv); return 1; }

  // 3) Permission checks: final dir requires write/exec for create
  snprintf(p, sizeof(p), "%s/dir", mnt); if (mkdir(p, 0755)!=0){ tlogf("mkdir dir failed:%s", strerror(errno)); stop_kafs(mnt, srv); return 1; }
  if (chmod(p, 0555)!=0){ tlogf("chmod failed:%s", strerror(errno)); stop_kafs(mnt, srv); return 1; }
  char p2[PATH_MAX]; snprintf(p2,sizeof(p2),"%s/dir/new", mnt);
  errno=0; int rc=open(p2,O_CREAT|O_WRONLY,0644); if(rc>=0){ close(rc); tlogf("create should have failed in ro dir"); stop_kafs(mnt, srv); return 1; }
  if (errno!=EACCES && errno!=EPERM){ tlogf("expected EACCES/EPERM, got %d", errno); stop_kafs(mnt, srv); return 1; }
