...
static int copy_file(const char *src, const char *dst) {
  ...
  while ((n = read(s, buf, sizeof(buf))) > 0) {
    ssize_t w = write(d, buf, (size_t)n); if (w != n) { int e=-errno; close(s); close(d); return e; }
  }
...
int main(void) {
  ...
  char tdir[PATH_MAX]; snprintf(tdir, sizeof(tdir), "%s/tmpl", mnt); if (mkdir(tdir, 0777)!=0){ tlogf("mkdir tmpl failed:%s", strerror(errno)); stop_kafs(mnt, srv); return 1; }
  char sfile[PATH_MAX]; snprintf(sfile,sizeof(sfile),"%s/tmpl/a.txt", mnt); int fd=open(sfile,O_CREAT|O_WRONLY,0644); if(fd<0){ tlogf("create a.txt failed:%s", strerror(errno)); stop_kafs(mnt, srv); return 1; } write(fd,"hello",5); close(fd);

  // create target dir and copy
  // ensure parent dirs
  char target[PATH_MAX]; snprintf(target, sizeof(target), "%s/target", mnt);
  if (mkdir(target, 0777)!=0 && errno!=EEXIST){ tlogf("mkdir target failed:%s", strerror(errno)); stop_kafs(mnt, srv); return 1; }
  char cmd[PATH_MAX]; snprintf(cmd, sizeof(cmd), "%s/target/.git", mnt);
  if (mkdir(cmd, 0777)!=0 && errno!=EEXIST){ tlogf("mkdir .git failed:%s", strerror(errno)); stop_kafs(mnt, srv); return 1; }
  char ddir[PATH_MAX]; snprintf(ddir, sizeof(ddir), "%s/target/.git/hooks", mnt);
  if (mkdir(ddir, 0777)!=0 && errno!=EEXIST){ tlogf("mkdir hooks failed:%s", strerror(errno)); stop_kafs(mnt, srv); return 1; }

  char dst[PATH_MAX]; snprintf(dst,sizeof(dst),"%s/target/.git/hooks/a.txt", mnt);
  int rc = copy_file(sfile, dst);
  if (rc != 0) { tlogf("copy failed:%s", strerror(-rc)); stop_kafs(mnt, srv); return 1; }

  // verify content
  char buf[16]={0}; fd=open(dst,O_RDONLY); if(fd<0){ tlogf("open dst failed:%s", strerror(errno)); stop_kafs(mnt, srv); return 1; }
  read(fd, buf, sizeof(buf)); close(fd);
  if (strncmp(buf, "hello", 5)!=0){ tlogf("content mismatch: %s", buf); stop_kafs(mnt, srv); return 1; }
