...
int main(void) {
  ...
  const char *img = "prune-di.img";
  const char *mnt = "mnt-prune-di";
  ...
  // Pick a logical block in the first double-indirect region: 12 + blkrefs_pb * 1
  // We can derive blkrefs_pb from superblock later, but we know for 4K block and 4-byte refs it's 1024
  // So choose iblo = 12 + 1024*0 + 0 => to ensure double-indirect tables are allocated. Then zero it.
  off_t p_off = (off_t)(12 + 1024) * bs;
  char *buf = malloc(bs); memset(buf, 0xCD, bs);
  ssize_t w = pwrite(fd, buf, bs, p_off); free(buf);
  if (w != (ssize_t)bs) { tlogf("pwrite DI data failed: %s", strerror(errno)); close(fd); stop_kafs(mnt, srv); return 1; }

  char *z = calloc(1, bs); w = pwrite(fd, z, bs, p_off); free(z);
  if (w != (ssize_t)bs) { tlogf("pwrite DI zero failed: %s", strerror(errno)); close(fd); stop_kafs(mnt, srv); return 1; }
  fsync(fd); close(fd);
  ...
  const char *name = "file"; size_t name_len = strlen(name);
  ...
  // After prune, i_blkreftbl[13] (double-indirect top table) may be NONE if its only child was cleared
  if (kafs_blkcnt_stoh(fileino->i_blkreftbl[13]) != 0) {
    tlogf("expected i_blkreftbl[13]==0 after prune, got non-zero");
    munmap(base, mapsize); close(ifd); return 1;
  }

  munmap(base, mapsize); close(ifd); tlogf("prune_indirect_double OK"); return 0;
}
