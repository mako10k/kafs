...
static inline int kafs_v6_admission_validate_descriptor_segments_fd(
...
static inline int kafs_v6_admission_preflight_core(int fd, const kafs_ssuperblock_t *sbdisk)
{
  if (fd < 0 || !sbdisk)
    return -EINVAL;
  ...
  if (rc == 0)
    rc = kafs_v6_admission_validate_descriptor_segments_fd(&preflight_ctx, fd, sbdisk, file_size);

  kafs_bitmap_descriptor_mapping_clear(&preflight_ctx);
  return rc;
}

static inline int kafs_v6_admission_runtime_context(kafs_context_t *ctx,
                                                    const kafs_ssuperblock_t *sbdisk, int prot)
{
  ...
  if (rc == 0)
    rc = kafs_v6_admission_validate_descriptor_segments_fd(ctx, ctx->c_fd, sbdisk, file_size);
  if (rc == 0)
